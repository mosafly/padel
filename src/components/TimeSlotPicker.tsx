...
const TimeSlotPicker: React.FC<TimeSlotPickerProps> = ({
  ...
  const generateTimeSlots = () => {
    ...
    for (let hour = startHour; hour < endHour; hour++) {
      ...
      const isPast = isSameDay(date, new Date()) && isBefore(startTime, new Date());
      
      // Check if slot is available (not conflicting with any reservation)
      const isAvailable = !isPast && availableSlots.some(slot => 
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
        {timeSlots.map((slot, index) => {
          const isSelected = selectedStartTime 
            ? format(selectedStartTime, 'HH:mm') === format(slot.startTime, 'HH:mm')
            : false;
            
          return (
            <button
              key={index}
              onClick={() => slot.isAvailable && onSelectTimeSlot(slot.startTime, slot.endTime)}
