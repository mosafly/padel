...
const FinancialTracking: React.FC = () => {
  ...
  const [courtRevenue, setCourtRevenue] = useState<{ name: string; value: number }[]>([]);
    ...
    const fetchFinancialData = async () => {
      try {
        ...
        for (let i = 0; i < monthsToFetch; i++) {
          ...
          const monthRevenue = revenueData?.reduce((acc, curr) => {
            return acc + (curr.total_price || 0);
          }, 0) || 0;
          
          const confirmedRevenue = revenueData?.filter(r => r.status === 'confirmed')
            .reduce((acc, curr) => acc + (curr.total_price || 0), 0) || 0;
            
          const pendingRevenue = revenueData?.filter(r => r.status === 'pending')
            .reduce((acc, curr) => acc + (curr.total_price || 0), 0) || 0;
            
          const cancelledRevenue = revenueData?.filter(r => r.status === 'cancelled')
            .reduce((acc, curr) => acc + (curr.total_price || 0), 0) || 0;
        ...
        courtData?.forEach(item => {
          const courtId = item.courts.id;
          const courtName = item.courts.name;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 mb-1">Financial Tracking</h1>
          <p className="text-gray-600">Monitor revenue and financial performance</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-blue-50 p-6 rounded-lg shadow-sm">
          <div className="flex items-center mb-2">
            ...
            <h3 className="text-sm font-medium text-gray-700 ml-1">Total Revenue</h3>
          </div>
        ...
        <div className="bg-green-50 p-6 rounded-lg shadow-sm">
          <div className="flex items-center mb-2">
            ...
            <h3 className="text-sm font-medium text-gray-700 ml-1">Confirmed Revenue</h3>
          </div>
          <p className="text-2xl font-bold text-gray-900">${revenueByStatus.confirmed.toFixed(2)}</p>
          <p className="text-sm text-gray-600 mt-1">
        ...
        <div className="bg-yellow-50 p-6 rounded-lg shadow-sm">
          <div className="flex items-center mb-2">
            ...
            <h3 className="text-sm font-medium text-gray-700 ml-1">Pending Revenue</h3>
          </div>
          <p className="text-2xl font-bold text-gray-900">${revenueByStatus.pending.toFixed(2)}</p>
          <p className="text-sm text-gray-600 mt-1">
              ...
              {[1, 2, 3, 4, 5].map((_, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap">
                    {index % 3 === 0 && <span className="badge badge-success">Confirmed</span>}
                    {index % 3 === 1 && <span className="badge badge-warning">Pending</span>}
                    {index % 3 === 2 && <span className="badge badge-danger">Cancelled</span>}
                  </td>
                </tr>
              ))}
            </tbody>
