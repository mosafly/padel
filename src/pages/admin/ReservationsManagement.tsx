...
const ReservationsManagement: React.FC = () => {
  ...
  useEffect(() => {
    const fetchReservations = async () => {
      try {
        ...
        const courtIds = [...new Set(resData!.map(r => r.court_id))];
        ...
        const transformed = resData!.map((r: any) => ({
          ...
          status: r.status,
          court_id: r.court_id,
          court_name: courtsData?.find(c => c.id === r.court_id)?.name || '',
          user_id: r.user_id,
          user_email: profilesData?.find(p => p.id === r.user_id)?.email || '',
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 mb-1">Reservations Management</h1>
