...
import { Brackets as Racket, CalendarCheck, DollarSign, Users } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const { data: reservationsDataRaw, error: reservationsError } = await supabase
          .from('reservations')
          .select('*')
        ...
        const totalRevenue = revenueDataRaw?.reduce((acc, curr) => acc + (curr.total_price || 0), 0) || 0;
        ...
        for (let i = 6; i >= 0; i--) {
          ...
          if (dayError) {
            revenuePerDay.push({ date: format(new Date(start), 'MMM dd'), revenue: 0 });
          } else {
            const total = dayData?.reduce((acc, curr) => acc + (curr.total_price || 0), 0) || 0;
            revenuePerDay.push({ date: format(new Date(start), 'MMM dd'), revenue: total });
          }
        }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Overview of reservations and business metrics</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat, index) => (
          <div 
            key={index} 
            className={`${stat.bgColor} p-6 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200`}
          >
            <div className="flex items-center mb-2">
              <div className="text-[var(--primary)]">
                {stat.icon}
              </div>
              <h3 className="ml-2 text-sm font-medium text-gray-700">{stat.label}</h3>
            </div>
          ...
          <div className="space-y-3">
            {reservationsData && reservationsData.length > 0 ? (
              reservationsData.slice(0, 5).map((reservation: Reservation) => (
                <div key={reservation.id} className="flex items-center justify-between p-3 border-b border-gray-100">
                  <div>
          ...
          <div className="space-y-3">
            {courtsData && courtsData.length > 0 ? (
              courtsData.map((court: any) => (
                <div key={court.id} className="flex items-center justify-between p-3 border-b border-gray-100">
                  <p className="font-medium">{court.name}</p>
                  <span className={`badge ${
                    court.status === 'available' ? 'badge-success' : 
                    court.status === 'maintenance' ? 'badge-danger' : 'badge-accent'
                  }`}>
