...
const ReservationPage: React.FC = () => {
  ...
  console.log('VITE_LOMI_SECRET_KEY available?', !!import.meta.env.VITE_LOMI_SECRET_KEY);
  console.log('VITE_LOMI_SECRET_KEY value (masked):', import.meta.env.VITE_LOMI_SECRET_KEY ? `${import.meta.env.VITE_LOMI_SECRET_KEY.substring(0, 10)}...` : 'undefined');
  ...
  const handleOnlinePayment = async () => {
    ...
    try {
      const hours = (selectedEndTime.getTime() - selectedStartTime.getTime()) / (1000 * 60 * 60);
      const totalPrice = court.price_per_hour * hours;
      
      console.log('Starting online payment process with total price:', totalPrice);
      ...
      toast.error(`Paiement en ligne échoué: ${err?.message || 'Erreur inconnue'}`);
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center py-12">
        <div className="animate-pulse text-gray-500">Loading court details...</div>
      </div>
    );
  }
        ...
        <div className="md:flex">
          ...
          <div className="p-6 md:w-2/3">
            ...
            <div className="mt-4 flex items-center text-gray-700">
              ...
              <span className="text-lg font-medium">${court.price_per_hour.toFixed(2)}/hour</span>
            </div>
          </div>
        </div>
      </div>
