...
const PaymentSimulationPage: React.FC = () => {
    ...
    const processPayment = async () => {
      ...
      try {
        ...
        if (paymentFetchError) {
          console.error('Erreur lors de la récupération du paiement:', paymentFetchError);
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded-lg shadow-md">
      <h1 className="text-2xl font-bold text-center mb-6">Simulation de Paiement</h1>
      
      <div className="mb-6">
        <p className="text-gray-600 mb-2">Montant:</p>
        <p className="text-xl font-semibold">{(parseInt(amount) / 100).toFixed(2)} {currency}</p>
      </div>
      
      {reservationId && (
        <div className="mb-6">
          <p className="text-gray-600 mb-2">Réservation ID:</p>
          <p className="text-sm font-mono bg-gray-100 p-2 rounded">{reservationId}</p>
        </div>
      )}
