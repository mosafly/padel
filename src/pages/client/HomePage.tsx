...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    if (isConnected) {
      console.log('Connection state changed to connected, refreshing courts...');
  ...
  }, [fetchCourts, supabase, isConnected]);
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Find Your Court</h1>
      ...
      {isLoading ? (
        <div className="flex justify-center py-12">
          <div className="text-center">
            ...
            <p className="text-xs text-gray-500 mt-1">
              {retryCount > 0 ? `Retry attempt ${retryCount}` : "Connecting to database..."}
            </p>
          </div>
        </div>
      ) : error ? (
        <div className="text-center py-12 bg-white rounded-lg shadow-sm p-6" data-component-name="HomePage">
          <p className="text-red-500 mb-4">{error}</p>
          {error.includes('auth') || error.includes('credentials') || error.includes('session') ? (
            <div>
              <p className="text-gray-600 mb-4">Vous devez être connecté pour voir les courts.</p>
              <button 
        ...
        <div className="text-center py-12 bg-white rounded-lg shadow-sm">
          <p className="text-gray-500">
            {searchQuery ? 'No courts found with your search criteria.' : 'No courts available at the moment.'}
          </p>
