...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { amount, currency, callback_url, payment_methods = ["wave"] } = await req.json();

    if (!amount || !currency || !callback_url) {
      throw new Error("Missing required parameters");
    }

    console.log("Creating Lomi session with params:", { amount, currency, callback_url, payment_methods });
